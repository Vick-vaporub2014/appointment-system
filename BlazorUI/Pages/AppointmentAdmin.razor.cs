...
{
    public partial class AppointmentAdmin : ComponentBase
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            {
                appointments = result.Data;
                OnSearchUserChanged(new ChangeEventArgs { Value = "" }); // Initialize the filtered list with all appointments
                ApplyFilter();
            }
            else
            {
        ...
        // private void OnDateCahnged(ChangeEventArgs e)
        // {
        //     // Update the dateTimeString with the new value from the input
        //     createAppointment.DateTimeString = e.Value?.ToString() ?? string.Empty;
        //     // Notify the EditContext that the DateTime field has changed so it can re-validate and update the UI
        //     editContext.NotifyFieldChanged(editContext.Field(nameof(createAppointment.DateTime)));
        // }
        ...
        void ConfirmDelete(int appointmentid)
        {
        ...
        {
            var result = await IAppointmentService.DeleteAppointmentAsync(appointmentIdToDelete);
            if (result.Success)
            {
                await Refresh();
                ApplyFilter();
            ...
            {
                ShowToast("Error al eliminada la cita", "error");
                Console.WriteLine(result.Message);
            }
